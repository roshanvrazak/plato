...
from . import db
from .pricing import cost_pence
...
async def _log_usage(
    ...
    cost = cost_pence(model, in_tok, out_tok) if status_code < 400 else Decimal("0")
    async with db.conn() as c:
        await c.execute(
            """
            INSERT INTO usage_log (tenant_id, model, input_tokens, output_tokens, status_code, cost_pence)
            VALUES ($1, $2, $3, $4, $5, $6)
            """,
            tenant.id, model, in_tok, out_tok, status_code, cost,
        )